...
function schnorrVerifySync(signature, message, publicKey) {
  try {
    const {r, s, m, P} = initSchnorrVerify(signature, message, publicKey);
    const e = schnorrChallengeFinalize(
      exports.utils.taggedHashSync(TAGS.challenge, numTo32b(r), P.toRawX(), m),
    );
    return finalizeSchnorrVerify(r, P, s, e);
  } catch (error) {
  ...
  randomBytes: (bytesLength = 32) => {
    if (crypto.web) {
      ...
      try {
        ...
        throw new Error(
          "The environment doesn't have randomBytes function and we couldn't use expo-crypto",
        );
      }
    }
  },
  randomPrivateKey: () =>
    exports.utils.hashToPrivateKey(exports.utils.randomBytes(groupLen + 8)),
