...
export class PubSub<E = string> {
  ...
  private removeSubscriber(event: E, cb: Function) {
  ...
  public subscribe(event: E, cb: (data: any) => void, meta?: any): () => void {
    if (!this.subscribers.has(event)) {
  ...
  public hasSubscribers(event: E): boolean {
    return (this.subscribers.get(event)?.callbacks?.length || 0) > 0;
  }
}
